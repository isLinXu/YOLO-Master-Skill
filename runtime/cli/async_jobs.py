...
class AsyncJobManager:
    ...
    def submit(
        ...
        request_path.write_text(
            json.dumps(child_request, ensure_ascii=False, indent=2), encoding="utf-8"
        )
        ...
        status = {
            ...
            "progress_path": str(
                (ensure_manifest_dir(child_request) / "progress.jsonl").resolve()
            ),
        }
    ...
    def status(
        self, job_id: str, request: dict[str, Any] | None = None
    ) -> dict[str, Any]:
