...
用法示例：
  ...
  snap.save("logs/snapshots/train_baseline.json")

  # 生产环境回放对比
  baseline = PayloadSnapshot.load("logs/snapshots/train_baseline.json")
...
class PayloadSnapshot:
    ...
    def __init__(
        self, skill: str, data: dict[str, Any], env: EnvironmentSnapshot | None = None
    ):
        self.skill = skill
        self.data = data
        self.env = env
        self.captured_at = data.get(
...
class DiffItem:
    def __init__(
        self, key: str, baseline: Any, current: Any, is_regression: bool, note: str = ""
    ):
        self.key = key
...
class SnapshotDiff:
    ...
    @classmethod
    def compare(
        cls, baseline: PayloadSnapshot, current: PayloadSnapshot
    ) -> "SnapshotDiff":
        items: list[DiffItem] = []
        ...
        for key in sorted(all_keys):
            ...
            if (
                ...
                note = (
                    "next_actions disappeared"
                    if is_regression
                    else "next_actions presence changed"
                )
            else:
                is_regression = False
        ...
        return cls(
            items=items, baseline_skill=baseline.skill, current_skill=current.skill
        )
    ...
    def summary(self) -> str:
        ...
        for it in self.items:
            ...
            lines.append(
                f"  [{tag}] {it.key}: {it.baseline!r} → {it.current!r}  ({it.note})"
            )
        return "\n".join(lines)
