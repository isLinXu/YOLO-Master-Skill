...
def expand_image_reference(value: Any, root: Path | None = None) -> list[Path]:
    ...
    if candidate.is_dir():
        return sorted(
            path.resolve() for path in candidate.rglob("*") if is_image_file(path)
        )
...
def load_dataset_yaml(data_ref: Any) -> tuple[Path, dict[str, Any]]:
    ...
    if data_ref in (None, ""):
        raise ValueError(
            "`inputs.data` or `params.data` is required when no image `source` is provided."
        )
...
def dataset_split_spec(
    dataset_cfg: dict[str, Any], requested_split: str
) -> tuple[str, Any]:
...
def read_ground_truth_summary(
    image_path: Path, names: dict[int, str], max_objects: int = 30
) -> dict[str, Any]:
