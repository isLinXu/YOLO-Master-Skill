...
from runtime.open_world.taxonomy import merge_verified_open_world_candidates  # noqa: E402
...
DEFAULT_PROMPT = (
    "Discover visible objects outside COCO and preserve them for open-world reasoning."
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run real open-world profile comparisons on a small image batch."
    )
    parser.add_argument(
        "--images", nargs="+", required=True, help="Image paths to evaluate."
    )
    parser.add_argument("--model", default="yolo11n.pt")
    parser.add_argument(
        "--profiles", nargs="+", default=["strict", "balanced", "exploratory"]
    )
    parser.add_argument("--prompt", default=DEFAULT_PROMPT)
    parser.add_argument("--vlm-model", default="qwen-vl-plus")
    parser.add_argument(
        "--openai-base-url", default="https://dashscope.aliyuncs.com/compatible-mode/v1"
    )
...
def run_case(
    *, image_path: Path, profile: str, args: argparse.Namespace, env: dict[str, str]
) -> dict[str, Any]:
    request = {
    ...
    payload = (
        json.loads(lines[-1])
        if lines
        else {"status": "failed", "summary": "no payload"}
    )
    comparison = (
        payload.get("multimodal", {}).get("open_world_comparison", {})
    ...
    return {
        ...
        "effective_prompt_template": payload.get("multimodal", {}).get(
            "effective_prompt_template"
        ),
        "open_world_assist_profile": payload.get("multimodal", {}).get(
...
def aggregate_profile(items: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for item in items:
        ...
        for ow in comparison.get("open_world_predictions", []) or []:
            label = (
                ow.get("canonical_open_label")
                or ow.get("open_label")
                or ow.get("label")
            )
            if ow.get("open_world_report_bucket") == "reasoning_only":
                if label:
                    reasoning_only_counts[str(label)] = (
                        reasoning_only_counts.get(str(label), 0) + 1
                    )
...
def render_item_labels(entries: list[dict[str, Any]]) -> str:
    ...
    for entry in entries:
        label = (
            entry.get("canonical_open_label")
            or entry.get("open_label")
            or entry.get("label")
            or "-"
        )
        ...
        suffix = (
            ...
            else f"reasoning-only:{reasons or 'filtered'}"
        )
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    for profile in ["strict", "balanced", "exploratory"]:
        ...
        for item in payload.get("items", []):
            comparison = item.get("open_world_comparison", {}) or {}
            labels = render_item_labels(
                comparison.get("open_world_predictions", []) or []
            )
    ...
    if report.get("verified_open_world_list"):
        ...
        for image, items in report.get("verified_open_world_list", {}).items():
            labels = (
                ", ".join(
                    (item.get("canonical_open_label") or item.get("open_label") or "-")
                    for item in items
                )
                or "-"
            )
...
def main() -> int:
    ...
    for profile in args.profiles:
        for raw_image in args.images:
            ...
            rows.append(
                run_case(image_path=image_path, profile=profile, args=args, env=env)
            )
