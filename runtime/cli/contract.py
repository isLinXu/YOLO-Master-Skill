    ...
    if isinstance(value, dict):
        ...
        for key, child in value.items():
            if (
                ...
                and len(child) > MANIFEST_LOG_LIMIT
            ):
...
def _atomic_write_json(path: Path, payload: dict[str, Any]) -> None:
    ...
    descriptor, temporary_name = tempfile.mkstemp(
        prefix=".skill_manifest.", suffix=".tmp", dir=path.parent
    )
...
def normalize_token_usage(raw_usage: dict[str, Any]) -> dict[str, int]:
    ...
    output_tokens = _token_value(
        raw_usage, "output", "output_tokens", "completion_tokens"
    )
...
def _pricing_for(provider: Any, model: Any) -> dict[str, float | None]:
    ...
    if not isinstance(model_pricing, dict):
        model_pricing = (
            pricing.get("default") if isinstance(pricing.get("default"), dict) else {}
        )
    return {
...
def write_manifest(request: dict[str, Any], payload: dict[str, Any]) -> Path:
    ...
    manifest = {
        ...
        **compatibility_fields,
        ...
        "result": _manifest_safe(
            {
                key: value
                for key, value in payload.items()
                if key not in represented_fields
            }
        ),
    }
...
def plan_response(
    ...
    payload = response(
        ...
        plan={
            ...
            "params": json_safe(
                params if params is not None else request.get("params", {})
            ),
        },
