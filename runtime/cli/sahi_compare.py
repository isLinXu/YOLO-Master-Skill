...
def run_sahi_compare(request: dict[str, Any]) -> dict[str, Any]:
    ...
    for image_path in images:
        ...
        if legacy_sahi is not None:
            start = time.perf_counter()
            boxes, scores, classes, meta = legacy_sahi.predict_standard(
                str(image_path), conf_thres=conf
            )
            item["full_sahi"] = {
                "latency_sec": round(
                    float(meta.get("inference_time", time.perf_counter() - start)), 6
                )
                if isinstance(meta, dict)
                else round(time.perf_counter() - start, 6),
                "boxes": len(boxes),
                "slices": len(meta.get("slices", []))
                if isinstance(meta, dict)
                else None,
                "classes": json_safe([int(value) for value in classes])
                if classes is not None
                else [],
                "scores": json_safe([float(value) for value in scores])
        ...
        item["sparse_sahi"] = {
            ...
            "slices": len(sparse_meta.get("slices", []))
        ...
        item["delta"] = {
            ...
            "sparse_vs_standard_speedup": round(base_latency / sparse_latency, 6)
            if sparse_latency > 0
            else None,
            "sparse_box_delta": int(item["sparse_sahi"]["boxes"])
            - int(item["standard"]["boxes"]),
        }
        items.append(item)
    ...
    payload = response(
        ...
        evaluation={
            "images": len(items),
            "avg_standard_latency_sec": round(
                sum(item["standard"]["latency_sec"] for item in items) / len(items), 6
            ),
            "avg_sparse_sahi_latency_sec": round(
                sum(item["sparse_sahi"]["latency_sec"] for item in items) / len(items),
                6,
            ),
        },
