...
def _weights(attr: Any) -> list[tuple[str, Any]]:
    ...
    if nn is not None and isinstance(attr, nn.ModuleDict):
        return [
            (str(name), child.weight)
            for name, child in attr.items()
...
def _adapter_value(
    value: Any, adapter: str, default: float | None = None
) -> float | None:
...
def _spectrum_for_delta(delta: Any, *, topk: int) -> dict[str, Any] | None:
    try:
        ...
        threshold_rank = (
            int((singular > singular[0] * 0.01).sum().item())
            if singular[0].item() > 0
            else 0
        )
        return {
            ...
            "singular_values_topk": [
                round(float(v), 6) for v in singular[:topk].tolist()
            ],
...
def delta_weight_spectrum(
    model: Any, *, max_layers: int = 12, topk: int = 8
) -> dict[str, Any]:
    layers = []
    skipped = 0
    ...
    if not layers:
        return {
            "layers": [],
            "summary": {"sampled_layers": 0, "skipped_layers": skipped},
        }
    return {
        "layers": layers,
        "summary": {
            ...
            "rank_1pct_avg": round(
                sum(layer["rank_1pct"] for layer in layers) / len(layers), 6
            ),
        },
    }


def run_lora_diagnose(
    request: dict[str, Any], deps: LoraDiagnoseDeps
) -> dict[str, Any]:
    ...
    stats = get_lora_training_stats(
        base_model, svd_sample_ratio=svd_sample_ratio, svd_max_layers=svd_max_layers
    )
    ...
    payload = deps.response(
        ...
        data=report,
        metrics={
            "effective_rank_avg": stats.get("effective_rank_avg"),
            "lora_modules": stats.get("lora_modules"),
        },
