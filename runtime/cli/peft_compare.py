...
DEFAULT_VARIANTS = [
    ...
    {
        "name": "loha_r16",
        "train": {"lora_type": "loha", "lora_r": 16, "lora_alpha": 32},
    },
]
    ...
    if isinstance(variants, list) and variants:
        ...
        for idx, item in enumerate(variants):
            if isinstance(item, str):
                ...
                normalized.append(
                    {"name": item.get("name") or f"variant_{idx + 1}", **item}
                )
        return normalized
...
def _score_payload(payload: dict[str, Any], metric_path: str) -> float | None:
    for root in ("evaluation", "metrics", "data"):
        value = _nested_get(
            payload.get(root, {}) if isinstance(payload.get(root), dict) else {},
            metric_path,
        )
        if value is None:
            value = (
                (payload.get(root) or {}).get(metric_path)
...
def _variant_train_skill(
    train_params: dict[str, Any], explicit: str | None = None
) -> str:
    if explicit:
        return explicit
    try:
        ...
        rank = 0
    return (
        "yolo.lora.train" if rank > 0 or train_params.get("lora_type") else "yolo.train"
    )


def _preview_variant(
    base_train: dict[str, Any], base_val: dict[str, Any], variant: dict[str, Any]
) -> dict[str, Any]:
    train_params = {
        **base_train,
        **dict(variant.get("train") or variant.get("params") or {}),
    }
...
def run_peft_compare(request: dict[str, Any], deps: PeftCompareDeps) -> dict[str, Any]:
        ...
        results.append(
            {
                ...
                "status": val_payload.get("status")
                if val_payload
                else train_payload.get("status"),
                "score": score,
    ...
    payload = deps.response(
        ...
        peft_compare={
            ...
            "ranking": [
                {"name": item["name"], "score": item["score"]} for item in ranked
            ],
        },
        results=results,
