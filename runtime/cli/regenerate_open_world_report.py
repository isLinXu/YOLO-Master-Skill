...
def enrich_record(
    module: Any, record: dict[str, Any], multimodal_params: dict[str, Any]
) -> dict[str, Any]:
    ...
    possible_misses = module.normalize_possible_miss_items(
        record.get("possible_misses"), multimodal_params
    )
    false_positives = module.normalize_possible_miss_items(
        record.get("false_positives"), multimodal_params
    )
    fusion_summary = (
        record.get("fusion_summary")
        if isinstance(record.get("fusion_summary"), dict)
        else {}
    )
    return {
        ...
        "vlm_labels": record.get("vlm_labels")
        if isinstance(record.get("vlm_labels"), list)
        else [],
...
def build_report(module: Any, records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for profile, items in sorted(by_profile.items()):
        profiles[profile] = {
            ...
            "parsed": sum(
                1 for item in items if item.get("verdict_parse_status") == "parsed"
            ),
        }
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    for profile, payload in profiles.items():
        ...
        for item in items:
            ...
            lines.append(
                f"- `{image}`: open-world=`{ow_labels}`; possible-misses=`{miss_labels}`"
            )
        lines.append("")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Regenerate taxonomy-enriched open-world small-batch reports."
    )
