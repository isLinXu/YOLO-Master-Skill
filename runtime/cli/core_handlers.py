...
def run_train_like(
    request: dict[str, Any], skill_name: str, deps: CoreDeps
) -> dict[str, Any]:
    ...
    params, chosen_device, auto_completed = apply_runtime_defaults(
        request, params, purpose="train"
    )
    ...
    if prefer_cli(request):
        ...
        if save_dir and save_dir.exists():
            ...
            if best.exists():
                artifacts.append(
                    {"kind": "checkpoint", "label": "best", "path": str(best.resolve())}
                )
            if last.exists():
                artifacts.append(
                    {"kind": "checkpoint", "label": "last", "path": str(last.resolve())}
                )
            if (save_dir / "results.csv").exists():
                artifacts.append(
                    {"kind": "csv", "path": str((save_dir / "results.csv").resolve())}
                )
            if (save_dir / "args.yaml").exists():
                artifacts.append(
                    {"kind": "config", "path": str((save_dir / "args.yaml").resolve())}
                )
        return response(
            skill_name,
            "ok",
...
def run_val(request: dict[str, Any], deps: CoreDeps) -> dict[str, Any]:
    ...
    if prefer_cli(request):
        ...
        if save_dir and (save_dir / "predictions.json").exists():
            artifacts.append(
                {"kind": "json", "path": str((save_dir / "predictions.json").resolve())}
            )
        return response(
            request["skill"],
            "ok",
...
def run_predict_like(
    request: dict[str, Any], mode: str, deps: CoreDeps
) -> dict[str, Any]:
    params = dict(request["params"])
    ...
    if prefer_cli(request):
        ...
        if save_dir and save_dir.exists():
            payload["artifacts"] = [
                {"kind": "directory", "path": str(save_dir.resolve())}
            ]
        return payload
...
def run_benchmark(request: dict[str, Any], deps: CoreDeps) -> dict[str, Any]:
    ...
    device_selection = resolve_device_selection(request, params)
    params, chosen_device, auto_completed = apply_runtime_defaults(
        request, params, purpose="benchmark"
    )
    effective_request = deepcopy(request)
    effective_request["params"] = params
    if is_dry_run(request):
...
def run_lora_adapters(request: dict[str, Any], deps: CoreDeps) -> dict[str, Any]:
    ...
    if action == "save":
        ...
        payload = response(
            request["skill"], "ok" if ok else "failed", "adapter load finished"
        )
    elif action == "merge":
        ok = model.merge_lora()
        payload = response(
            request["skill"], "ok" if ok else "failed", "adapter merge finished"
        )
    else:
