...
def parse_moe_prune_stdout(
    stdout: str, *, original_model: Any, pruned_model: Any, threshold: float
) -> dict[str, Any]:
    ...
    for raw_line in stdout.splitlines():
        ...
        experts_match = re.search(
            r"Experts:\s*(\d+)\s*(?:\u2192|->)\s*(\d+).*keeping\s*\[([^\]]*)\]", line
        )
    ...
    prune_ratio = (
        round((before_total - after_total) / before_total, 6) if before_total else 0.0
    )
    validation_after = (
        {"status": "ok"} if "Validation check: OK" in stdout else {"status": "unknown"}
    )
    ...
    return {
        ...
        "validation_before": {
            "status": "not_run",
            "reason": "underlying pruner only validates the pruned checkpoint",
        },
