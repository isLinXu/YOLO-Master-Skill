...
class StabilityChecker:
    ...
    def _require_field(
        self, path: str, types: tuple | None = None, rule_prefix: str = "schema"
    ) -> bool:
    ...
    def _require_in(
        self, path: str, allowed: set, rule_prefix: str = "behavior"
    ) -> bool:
        v = self._get(path)
        ok = v in allowed
        if ok:
    ...
    def _require_range(
        self, path: str, lo: float, hi: float, rule_prefix: str = "monotonic"
    ) -> bool:
        v = _to_float(self._get(path))
        if v is None:
            self._add(
                _fail(f"{rule_prefix}.range", path, "field missing or non-numeric")
            )
            return False
        ok = lo <= v <= hi
        if ok:
    ...
    def _require_nonneg(self, path: str, rule_prefix: str = "monotonic") -> bool:
        ...
        if v is None:
            self._add(
                _fail(f"{rule_prefix}.nonneg", path, "field missing or non-numeric")
            )
            return False
        ok = v >= 0
        if ok:
    ...
    def _require_nonempty(self, path: str, rule_prefix: str = "schema") -> bool:
        ...
        if ok:
            ...
            self._add(
                _fail(f"{rule_prefix}.nonempty", path, f"empty or missing: {v!r}")
            )
        return ok
    ...
    def _require_implies(
        ...
        cond_val: Any,
        then_path: str,
        rule: str = "behavior.implication",
    ) -> bool:
        """如果 cond_path == cond_val，则 then_path 必须存在且非空。"""
        cond = self._get(cond_path)
        if cond != cond_val:
            self._add(
                _ok(rule, then_path, f"condition {cond_path}={cond!r} not met, skip")
            )
            return True
    ...
    def _check_universal_behavior(self) -> None:
        ...
        if _is_dict(recovery):
            ...
            if not isinstance(attempted, bool):
                ...
                self._add(
                    _ok("behavior.recovery_attempted_is_bool", "recovery.attempted")
                )
    ...
    def _check_train_val_monotonic(self) -> None:
        ...
        for k in ("preprocess", "inference", "postprocess"):
            ...
            if v is not None:
                ...
                if ok:
                    self._add(
                        _ok("monotonic.nonneg", f"evaluation.speed_ms.{k}", f"{v}")
                    )
                else:
                    self._add(
                        _fail(
                            "monotonic.nonneg", f"evaluation.speed_ms.{k}", f"{v} < 0"
                        )
                    )
    ...
    def _check_train_behavior(self) -> None:
        ...
        has_ckpt = any(
            isinstance(a, dict) and a.get("kind") == "checkpoint" for a in arts
        )
        if has_ckpt:
    ...
    def _check_multimodal_behavior(self) -> None:
        ...
        if status == "blocked":
            ...
            if v:
                self._add(
                    _ok("behavior.blocked_has_reason", "multimodal.vlm.blocked_reason")
                )
            else:
                self._add(
                    _fail(
    ...
    def _check_multimodal_evaluate_schema(self) -> None:
        ...
        if _is_dict(mg):
            if mg.get("selected") not in (None, "yolo_only", "fused_preview"):
                ...
                self._add(
                    _ok("behavior.guardrail_selected_enum", "metric_guardrail.selected")
                )
    ...
    def _check_multimodal_evaluate_monotonic(self) -> None:
        ...
        if ip is not None:
            ...
            if ok:
                self._add(
                    _ok("monotonic.nonneg", "evaluation.images_processed", f"{ip}")
                )
            else:
                self._add(
                    _fail(
                        "monotonic.nonneg", "evaluation.images_processed", f"{ip} < 0"
                    )
                )
        ...
        if _is_dict(mg) and mg.get("selected") == "fused_preview":
            delta = _to_float(
                _dotted_get(self.payload, "evaluation.metric_preview.delta.map50_95")
            )
    ...
    def _check_lora_diagnose_schema(self) -> None:
        ...
        for f in ("adapter_path", "layers_analyzed"):
            if f not in diag:
                self._add(
                    _fail("schema.required", f"lora_diagnose.{f}", "field missing")
                )
            else:
        ...
        if er is not None:
            ...
            if ok:
                ...
                self._add(
                    _fail("monotonic.nonneg", "lora_diagnose.effective_rank", msg)
                )
                ...
                for f in ("name", "rank_metric_value"):
                    if f not in item:
                        self._add(
                            _fail(
                                "schema.required",
                                f"peft_compare.ranking.{i}.{f}",
                                "field missing",
                            )
                        )
                    else:
                        self._add(
                            _ok("schema.required", f"peft_compare.ranking.{i}.{f}")
                        )
