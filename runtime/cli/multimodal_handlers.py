...
from runtime.cli.dataset import (
    collect_multimodal_evaluation_images,
    read_ground_truth_summary,
)
...
def run_multimodal_infer(
    request: dict[str, Any], deps: MultimodalDeps
) -> dict[str, Any]:
    ...
    source = (
        ...
        or yolo_params.pop("source", None)
        or multimodal_params.get("source")
    )
    if source is None:
    ...
    max_items = int(
        yolo_params.pop("max_items", multimodal_params.get("max_reasoning_items", 3))
    )
    ...
    resolved_prompt_template = effective_prompt_template_name(
        provider_cfg, prompt_template, multimodal_params, prompt
    )
    ...
    max_output_tokens = int(
        multimodal_params.get("max_output_tokens", default_max_output_tokens)
    )
    use_marked_image = parse_bool(
        multimodal_params.get("use_marked_image"), bool(resolved_prompt_template)
    )
    ...
    method = str(
        ...
        or (
            "thinking-with-image" if thinking_with_image else "detector-text-reflection"
        )
    )
        ...
        return plan_response(
            ...
            params={
                "stages": [
                    ...
                    },
                    {
                        ...
                        "enabled": visual_search_mode.lower()
                    ...
                    },
                    {
                        ...
                        "enabled": bool(
                            ...
                            or multimodal_params.get("enable_llm_refine")
                        ),
                    },
                ],
                "prompt": prompt,
            ...
            extra={
                ...
                "multimodal": {
                    ...
                    "structured_output": structured_output,
                    "prompt_template": resolved_prompt_template,
                    "open_world_assist_profile": multimodal_params.get(
                        "open_world_assist_profile"
                    ),
                    "open_world_filters": {
                        "taxonomy_min_score": multimodal_params.get(
                        ...
                        "filter_unmatched_taxonomy": multimodal_params.get(
                            "open_world_filter_unmatched_taxonomy"
                        ),
                        "filter_generic_labels": multimodal_params.get(
    ...
    if bool(multimodal_params.get("skip_yolo", False)):
        detections = json_safe(
            request["inputs"].get("detections")
            or multimodal_params.get("detections")
            or []
        )
    else:
        try:
            ...
            if device_selection["source"] == "auto" and chosen_device not in (
                None,
                "cpu",
            ):
        ...
        detections = summarize_results_for_reasoning(
            results, max_items=max_items, max_boxes=max_boxes
        )

    image_ref = visual_image_source_for_openai(
        source, detections, resolved_path=resolved_path
    )
    ...
    if thinking_with_image and image_ref is None:
        ...
        try:
            ...
            if thinking_with_image and image_ref is not None:
                ...
                if use_marked_image and not str(image_ref).startswith(
                    ...
                    try:
                        ...
                        visual_artifacts.append(
                            {"kind": "marked_image", "path": marked["path"]}
                        )
                    except Exception as mark_exc:
                        marked_error = {
                            "type": type(mark_exc).__name__,
                            "message": str(mark_exc),
                        }
                ...
                image_meta = {
                    k: v for k, v in encoded_image.items() if k != "image_url"
                }
            ...
            user_text = build_thinking_with_image_prompt(
                ...
                thinking_with_image=thinking_with_image,
                structured_output=structured_output,
                prompt_template=str(resolved_prompt_template)
            ...
            vlm_result = deps.call_openai_compatible(
                ...
                temperature=float(multimodal_params["temperature"])
                if "temperature" in multimodal_params
                else None,
            )
            vlm_result = attach_multimodal_verdict(vlm_result)
            if vlm_result.get("status") == "ok" and image_ref is not None:
                visual_search_passes, search_artifacts = (
                    runtime_run_visual_search_crop_passes(
    ...
    llm_enabled = bool(
        provider_cfg.get("llm_model") or multimodal_params.get("enable_llm_refine")
    )
    if llm_enabled and vlm_result.get("status") == "ok":
        ...
        llm_result = deps.call_openai_compatible(
            ...
            developer_text=default_llm_refine_developer_prompt(
    ...
    vlm_verdict = (
        vlm_result.get("verdict", {})
        if isinstance(vlm_result.get("verdict"), dict)
        else {}
    )
    llm_verdict = (
    ...
    open_world_comparison = build_open_world_comparison_entry(
        ...
        verdict=fusion_verdict,
        multimodal_params=multimodal_params,
        effective_prompt_template=str(resolved_prompt_template)
        if resolved_prompt_template is not None
        else None,
    )
    ...
    if save_dir:
        payload["artifacts"] = [
            {"kind": "directory", "path": str(Path(save_dir).resolve())}
        ]
...
def run_multimodal_evaluate(
    request: dict[str, Any], deps: MultimodalDeps
) -> dict[str, Any]:
    params = dict(request["params"])
    yolo_params, multimodal_params, evaluate_params = (
        split_yolo_multimodal_evaluate_params(params)
    )
    multimodal_params = apply_open_world_assist_profile_defaults(multimodal_params)
    prompt = multimodal_prompt_from_request(request, multimodal_params)
    provider_cfg = openai_config(multimodal_params)
    ...
    include_ground_truth = parse_bool(
        evaluate_params.get(
            ...
            evaluate_params.get("include_ground_truth"),
        ),
        False,
    )
    ...
    report_name = str(
        evaluate_params.get("report_name") or "multimodal-evaluation.json"
    )
    data_ref_for_baseline = (
        request["inputs"].get("data")
        or evaluate_params.get("data")
        or yolo_params.get("data")
    )
    device_selection = resolve_device_selection(request, yolo_params)
    yolo_params, chosen_device, auto_completed = apply_runtime_defaults(
        request, yolo_params, purpose="predict"
        ...
        return plan_response(
            ...
            params={
                "stages": [
                    {
                        "name": "collect_images",
                        "executor": "dataset_or_source_resolver",
                    },
                    {
                        "name": "yolo_predict_batch",
                        "executor": "python_api",
                        "target": "YOLO(...).predict",
                    },
                    {
                        "name": "marked_image_batch",
                        "executor": "pillow",
                        "enabled": use_marked_image,
                    },
    ...
    for index, image_path in enumerate(images):
        ...
        try:
            ...
            if (
                ...
                try:
                    model = deps.build_model(effective_request)
                    yolo_prediction = model.predict(
                        source=str(image_path), **retry_params
                    )
                    yolo_params = retry_params
                    selected_device = "cpu"
                    auto_completed["device"] = "cpu"
        ...
        detection_summary = (
            detections[0]
            if detections
            else {"path": str(image_path), "speed": {}, "detections": []}
        )
        image_ref = visual_image_source_for_openai(
            str(image_path), detections, resolved_path=resolved_path
        )
        image_meta: dict[str, Any] = {
            "requested": image_ref,
            "thinking_with_image": thinking_with_image,
            ...
            try:
                ...
                if thinking_with_image and image_ref is not None:
                    ...
                    encoded_image = encode_image_reference_for_openai_raw(
                        ...
                        resolved_path=resolved_path,
                        max_bytes=int(
                            multimodal_params.get("max_image_bytes", 20_000_000)
                        ),
                    )
                    image_meta = {
                        k: v for k, v in encoded_image.items() if k != "image_url"
        ...
        llm_verdict = (
            ...
            and isinstance(llm_result.get("verdict"), dict)
            else {}
        )
        ...
        if status in {"blocked", "partial", "failed"}:
            image_item["notes"] = [
                "See multimodal cross-check and detection summary for details."
            ]
        items.append(image_item)
    ...
    final_selection_source = (
        "recovery"
        if selected_device != chosen_device and selected_device == "cpu"
        else device_selection["source"]
    )
    environment = collect_environment_report(
        effective_request,
    ...
    open_world_report = {
        "items": [
            ...
            for item in items
            if isinstance(
                item.get("multimodal", {}).get("open_world_comparison", {}), dict
            )
        ]
    }
    open_world_report["aggregate"] = aggregate_open_world_comparison(
        open_world_report["items"]
    )
    ...
    artifacts.append(
        {
            ...
            "path": str(open_world_report_path.resolve()),
        }
    )
    if fusion_coco_records:
