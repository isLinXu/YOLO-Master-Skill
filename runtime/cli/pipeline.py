...
def split_stage_config(
    ...
    stage_params = dict(
        config.get("params") or {k: v for k, v in config.items() if k not in structural}
    )
...
def pipeline_preview(
    ...
    for stage in requested_stages(params):
        _, stage_params, _, _, skill_override = split_stage_config(
            stage_config(params, stage)
        )
        ...
        if stage == "train":
            skill = (
                ...
                or (
                    "yolo.lora.train"
                    if _int_value(stage_params.get("lora_r")) > 0
                    else "yolo.train"
                )
            )
        preview.append(
            {"stage": stage, "skill": skill, "params": json_safe(stage_params)}
        )
    return preview


def stage_artifacts(
    request: dict[str, Any], stage: str, override: dict[str, Any]
) -> dict[str, Any]:
...
def execute_stage(
    stage: str, skill: str, stage_request: dict[str, Any], deps: PipelineDeps
) -> dict[str, Any]:
    if stage == "system":
...
def run_experiment_pipeline(
    request: dict[str, Any], deps: PipelineDeps
) -> dict[str, Any]:
    ...
    if not stages:
        raise ValueError(
            "`params` must include at least one pipeline stage, e.g. train, val, export, or benchmark."
        )
    ...
    if deps.is_dry_run(request):
        ...
        return deps.plan_response(
            ...
            extra={
                "pipeline": {
                    "stage_order": [item["stage"] for item in preview],
    ...
    for stage in stages:
        ...
        stage_inputs, stage_params, stage_policy, artifacts_override, skill_override = (
            split_stage_config(raw_config)
        )
        ...
        if not skill:
            raise ValueError(f"Unsupported pipeline stage: {stage}")
        append_progress_event(
            progress_path, {"event": "stage_start", "stage": stage, "skill": skill}
        )
        events += 1
        ...
        append_progress_event(
            progress_path,
            {"event": "stage_end", "stage": stage, "status": payload.get("status")},
        )
        events += 1
    ...
    append_progress_event(
        progress_path,
        {"event": "pipeline_end", "status": status, "failed_stage": failed_stage},
    )
    events += 1
