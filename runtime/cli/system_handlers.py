...
def run_system(request: dict[str, Any], deps: SystemDeps) -> dict[str, Any]:
    ...
    if is_dry_run(request):
        ...
        if action == "doctor":
            ...
            environment = collect_environment_report(
                request, selected_device=selected_device
            )
        ...
        cli_map = {
            ...
            "settings.update": [
                ...
                *[
                    ...
                    for k, v in (
                        params.get("updates")
                        or {k: v for k, v in params.items() if k != "action"}
                    ).items()
                ],
            ],
        ...
        return plan_response(
            request,
            "system dry run prepared",
            "module",
            f"yolo.system::{action}",
            params=params,
        )
    ...
    if action == "install":
        ...
        return response(
            ...
            "ultralytics CLI installed"
            if install["returncode"] == 0
            else "ultralytics CLI install failed",
    ...
    if action == "doctor":
        ...
        if ensure_cli:
            ...
            install = {
                "status": "available" if cli_path else "missing",
                "path": cli_path,
            }
        selected_device = resolve_default_device(request, params)
        environment = collect_environment_report(
            request, selected_device=selected_device, cli_info=install
        )
        recommendations = doctor_recommendations(environment)
        return response(
            request["skill"],
    ...
    if action in {
        ...
        failed = ensure_cli_success(
            request, cli_result, f"system action `{action}` failed"
        )
        if failed:
            return failed
        if action == "help":
        ...
        if action == "version":
            match = re.search(
                r"\b\d+\.\d+\.\d+\b", f"{cli_result['stdout']}\n{cli_result['stderr']}"
            )
        ...
        if action == "settings.update":
            ...
            return response(
                ...
                "settings updated",
                data={
                    "settings": json_safe(dict(core["SETTINGS"])),
                    "updated": json_safe(updates),
                },
                logs=cli_logs(cli_result),
            )
        ...
        if action == "cfg.copy":
            new_file = ensure_manifest_dir(
                request
            ) / deps.default_cfg_file.name.replace(".yaml", "_copy.yaml")
            return response(
                request["skill"],
                "ok",
                "default cfg copied",
