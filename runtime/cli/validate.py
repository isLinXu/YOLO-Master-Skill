...
def _load_snapshot_tools():
    try:
        from runtime.cli.snapshot import (
            PayloadSnapshot,
            SnapshotDiff,
            EnvironmentSnapshot,
        )
...
def load_cases(path: str | Path) -> list[dict[str, Any]]:
    ...
    if source.is_dir():
        ...
        for file in sorted(source.glob("*.json")):
            ...
            if not isinstance(data, list):
                raise ValueError(
                    f"Case file must contain a list or a {{'cases': [...]}} object: {file}"
                )
            cases.extend(data)
        return cases
    ...
    if not isinstance(data, list):
        raise ValueError(
            f"Case file must contain a list or a {{'cases': [...]}} object: {source}"
        )
    return data
...
def apply_env_overrides(
    env: dict[str, str], overrides: dict[str, Any] | None
) -> dict[str, str]:
    if not overrides:
        return env
    ...
    *,
    ...
    if isinstance(snap_cfg, dict) and snap_cfg.get("record") and returncode == 0:
        ...
        if PayloadSnapshot is not None:
            skill = str(payload.get("skill", request.get("skill", "")))
            snap = PayloadSnapshot.capture(
                payload, skill=skill, env=EnvironmentSnapshot.capture()
            )
        ...
        if PayloadSnapshot is not None and SnapshotDiff is not None:
            ...
            if baseline_path.exists():
                try:
                    ...
                    skill = str(payload.get("skill", request.get("skill", "")))
                    current = PayloadSnapshot.capture(
                        payload, skill=skill, env=EnvironmentSnapshot.capture()
                    )
    ...
    for path in expect.get("path_exists", []):
        ...
        result["checks"].append(
            {"kind": "path_exists", "ok": ok, "path": path, "actual": value}
        )
        result["passed"] &= ok
        ...
        result["checks"].append(
            {
                "kind": "returncode",
                "ok": ok,
                "expected": int(expected_returncode),
                "actual": returncode,
            }
        )
        result["passed"] &= ok
...
def run_probe_case(case: dict[str, Any]) -> dict[str, Any]:
    ...
    if kind == "recovery_auto_retry":
        ...
        try:
            outcome = module.run_cli_with_recovery(
                ...
                {
                    "model": probe.get("model", "m.pt"),
                    "device": probe.get("device", "mps"),
                },
    ...
    if kind == "multimodal_stub":
        ...
        open_world_mode = (
            str(request.get("params", {}).get("prompt_template") or "")
            == "vlm_open_world_detection"
        )
        generic_taxonomy_probe = bool(
                        ...
                        json.dumps(
                            {
                                ...
                                "visual_evidence": [
                                    "a bus and visible grass are both present"
                                ],
                                "caption": {
                        ...
                        json.dumps(
                            {
                                ...
                                "visual_evidence": [
                                    "bus remains correct and the grass region is still visible"
                                ],
                                "caption": {
                                    ...
                                    "add_open_world_detections": [
                                        {
                                            "proposal_id": "ow1",
                                            "confidence": 0.9,
                                            "evidence": "verified grassy patch",
                                        }
                                    ],
                                    "add_vlm_detections": [],
                                    "suppress_yolo_indices": [],
                        ...
                        json.dumps(
                            {
                                "answer": "open-world vlm answer",
                                "visual_evidence": [
                                    "a bus and a traffic cone are both visible"
                                ],
                                "caption": {
                                    ...
                                    "add_open_world_detections": [
                                        {
                                            "proposal_id": "ow1",
                                            "confidence": 0.88,
                                            "evidence": "visible roadside cone",
                                        }
                                    ],
                                    "add_vlm_detections": [],
                                    "suppress_yolo_indices": [],
                                ...
                                "uncertainty": "low",
                                "recommended_next_actions": [
                                    "preserve open-world object for downstream reasoning"
                                ],
                            }
                        ),
                        json.dumps(
                            {
                                "answer": "open-world refined answer",
                                "visual_evidence": [
                                    "bus remains correct and the cone is still visible"
                                ],
                                "caption": {
                                    ...
                                    "add_open_world_detections": [
                                        {
                                            "proposal_id": "ow1",
                                            "confidence": 0.9,
                                            "evidence": "verified roadside cone",
                                        }
                                    ],
                                    "add_vlm_detections": [],
                                    "suppress_yolo_indices": [],
                                ...
                                "uncertainty": "low",
                                "recommended_next_actions": [
                                    "keep the novel object in open-world preview"
                                ],
                            }
                        ),
                    ]
        ...
        def fake_urlopen(request_obj, timeout=120):
            ...
            calls.append(
                {"url": request_obj.full_url, "body": body, "timeout": timeout}
            )
    ...
    if kind == "multimodal_batch_stub":
        ...
        with temporary_env(probe_env):
            ...
            try:
                payload = module.run_multimodal_evaluate(
                    module.normalize_request(request)
                )
            finally:
        ...
        result_payload = {
            ...
            "data": {
                ...
                "first_item": payload.get("results", [{}])[0]
...
def recommend(results: list[dict[str, Any]]) -> list[str]:
    ...
    if not failures:
        recs = [
            "All cases passed. Keep the current skill shape and expand the case set before adding new handlers."
        ]
        if slow_cases:
            recs.append(
                "Keep heavyweight cases in deep-smoke and use fast-smoke for tight iteration loops."
            )
        return recs
    ...
    if any(name.startswith("system_") for name in failed_names):
        recs.append(
            "Tighten the system-action router and keep lazy imports for CLI-less actions."
        )
    if any(name.endswith("_dry_run") for name in failed_names):
        recs.append(
            "Patch the dry-run plan output before touching real execution paths."
        )
    if any(name.startswith("inspect_") for name in failed_names):
        recs.append(
            "Simplify inspect-time model construction and keep path normalization strict."
        )
    if any(name.startswith("pipeline_") for name in failed_names):
        recs.append(
            "Refactor pipeline orchestration to surface stage-level errors and stage manifests."
        )
    if any(name.startswith("recovery_") for name in failed_names):
        recs.append(
            "Keep the recovery probes green when adjusting auto device selection or CLI failure handling."
        )
    if any(name.startswith("multimodal_") for name in failed_names):
        recs.append(
            "Keep the multimodal blocked-path, fake OpenAI probe, and thinking-with-image contract aligned."
        )
    if any(
        check["kind"] == "max_elapsed_sec" and not check["ok"]
        for r in failures
    ...
    if not recs:
        recs.append(
            "Review the failing cases and add more narrow assertions around returned artifacts and paths."
        )
    return recs
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="AutoTrain-style validator for the YOLO-Master skill."
    )
    parser.add_argument(
        "--cases",
        default=str(DEFAULT_CASES),
        help="Path to autotrain case JSON file or directory.",
    )
    parser.add_argument(
        "--suite",
        default="quick",
    ...
    parser.add_argument(
        "--out",
        default=str(REPORT_DIR / "autotrain-report.json"),
        help="Output report path.",
    )
    parser.add_argument(
        "--pretty", action="store_true", help="Pretty-print the summary report."
    )
    parser.add_argument(
        "--summary-only",
        action="store_true",
    ...
    report = {
        ...
        "slowest": [
            ...
            for item in sorted(
                results, key=lambda item: item["elapsed_sec"], reverse=True
            )[:5]
        ],
        "results": results,
