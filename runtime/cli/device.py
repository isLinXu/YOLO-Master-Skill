...
def runtime_cache_enabled() -> bool:
    return os.environ.get("YOLO_MASTER_AGENT_RUNTIME_CACHE", "").lower() in {
        "1",
        "true",
        "yes",
    }
...
def read_torch_runtime_cache() -> dict[str, Any] | None:
    ...
    try:
        ...
        return None
    if (
        payload.get("python") != sys.executable
        or payload.get("platform") != sys.platform
    ):
        return None
...
def collect_environment_report(
    ...
    *,
    ...
    requested = (
        requested_device if requested_device is not None else selection["device"]
    )
    ...
    report = {
        ...
        "references": {
            ...
            "data": reference_state(
                request.get("inputs", {}).get("data")
                or request.get("params", {}).get("data")
            ),
...
def doctor_recommendations(environment: dict[str, Any]) -> list[str]:
    ...
    if not cli.get("available"):
        recommendations.append(
            "Run `python -m pip install -e .` to provision the local `yolo` CLI."
        )
    ...
    if (
        sys.platform == "darwin"
        and torch_info.get("mps", {}).get("available")
        and devices.get("selected") != "mps"
    ):
        recommendations.append(
            "This host supports MPS; leave `device` unset or set `runtime.prefer_mps=true` for Apple Silicon acceleration."
        )
    if (
        devices.get("selected") == "cpu"
        and sys.platform == "darwin"
        and not torch_info.get("mps", {}).get("available")
    ):
        recommendations.append(
    ...
    for label, state in (environment.get("references") or {}).items():
        if state.get("requested") not in (None, "") and not state.get("exists"):
            recommendations.append(
                f"Fix the `{label}` reference before launch: {state['requested']}"
            )
...
def apply_runtime_defaults(
    ...
    *,
    ...
    runtime = request.get("runtime", {})
    if (
        purpose in {"train", "val", "benchmark"}
