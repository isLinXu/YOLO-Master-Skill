...
def _resolve_profile_input(
    ...
    if not isinstance(profile_id, str) or not profile_id.strip():
        raise ValueError(
            "`inputs.profile` must be a non-empty mixture profile identifier."
        )
    ...
    if (
        requested_task
        and profile.task != "unknown"
        and str(requested_task).casefold() != profile.task.casefold()
    ):
        raise ValueError(
    ...
    provenance.update(
        {"model_path": str(model_path), "sha256": _sha256_file(model_path)}
    )
    return inputs, provenance


def _resolve_request_profile(
    inputs: dict[str, Any], existing: Any
) -> tuple[dict[str, Any], dict[str, Any] | None]:
    ...
    if not isinstance(existing, dict) or not existing.get("profile_id"):
        raise ValueError(
            "top-level profile provenance must contain a valid `profile_id`."
        )
    ...
    if supplied_model:
        ...
        if supplied_path.resolve() != Path(resolved_inputs["model"]).resolve():
            raise ValueError(
                "top-level profile provenance conflicts with the resolved model path."
            )
    ...
    if isinstance(value, str) and path_like(value):
        ...
        if not p.is_absolute():
            if value.startswith("agent/"):
                return str((SKILL_ROOT / value.removeprefix("agent/")).resolve())
            if value.startswith("assets/"):
                return str((SKILL_ROOT / value).resolve())
...
def normalize_request(request: dict[str, Any]) -> dict[str, Any]:
    ...
    request["inputs"], profile = _resolve_request_profile(
        request["inputs"], existing_profile
    )
...
def split_yolo_and_multimodal_params(
    params: dict[str, Any],
