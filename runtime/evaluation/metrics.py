...
def polygon_points_from_segment(
    segment: list[float], width: int, height: int
) -> list[list[float]]:
...
def polygon_iou_approx(
    a: list[list[float]], b: list[list[float]], samples: int = 24
) -> float:
...
def ground_truth_records_for_metric(
    image_path: Path, names: dict[int, str]
) -> list[dict[str, Any]]:
...
def ground_truth_classification_records_for_metric(
    image_path: Path, names: dict[int, str]
) -> list[dict[str, Any]]:
    label_path = label_path_for_image(image_path)
...
def ground_truth_segmentation_records_for_metric(
    image_path: Path, names: dict[int, str]
) -> list[dict[str, Any]]:
    label_path = label_path_for_image(image_path)
    image_size = image_size_for_metric(image_path)
...
def normalize_global_classification_items(
    verdict: dict[str, Any],
) -> list[dict[str, Any]]:
    items = (
        verdict.get("global_classification", []) if isinstance(verdict, dict) else []
    )
    ...
    normalized.sort(
        key=lambda entry: float(entry.get("confidence") or 0.0), reverse=True
    )
    return normalized
...
def yolo_prediction_records_for_metric(
...
def fused_prediction_records_for_metric(
    image_path: Path, fusion_preview: dict[str, Any]
) -> list[dict[str, Any]]:
    records: list[dict[str, Any]] = []
...
def yolo_classification_predictions_for_metric(
    ...
    for item in normalize_detection_boxes(detections):
        ...
        if current is None or float(confidence or 0.0) > float(
...
def match_class_predictions(
    ...
    *,
    ...
    class_ground_truth = [
        item for item in ground_truth if coerce_int(item.get("class_id")) == class_id
    ]
    ...
    *,
    ...
    gt_classes = sorted(
        {
            ...
            if coerce_int(item.get("class_id")) is not None
        }
    )
    pred_classes = sorted(
        {
    ...
    for threshold in thresholds:
        ...
        for class_id in all_classes:
            result = match_class_predictions(
                predictions, ground_truth, class_id=class_id, iou_threshold=threshold
            )
        ...
        per_threshold[f"{threshold:.2f}"] = {
            **threshold_counts,
            ...
            "map": round(
                sum(ap_by_threshold[threshold]) / len(ap_by_threshold[threshold]), 6
            )
    ...
    f1 = (
        (2 * precision_50 * recall_50 / (precision_50 + recall_50))
        if precision_50 + recall_50
        else 0.0
    )
    return {
        "status": "ok",
...
def metric_delta(fused: dict[str, Any], yolo: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in ("precision", "recall", "f1", "map50", "map50_95"):
        if isinstance(fused.get(key), (int, float)) and isinstance(
            yolo.get(key), (int, float)
        ):
    ...
    if "map50_95" in delta:
        delta["direction"] = (
            "improved"
            if delta["map50_95"] > 0
            else ("regressed" if delta["map50_95"] < 0 else "unchanged")
        )
    return delta
...
def evaluate_classification_metric_preview(
    ...
    return {
        ...
        "exact_set_accuracy": round(
            exact_match / total_images if total_images else 0.0, 6
        ),
        "precision": round(precision, 6),
        "recall": round(recall, 6),
        "f1": round(f1, 6),
...
def classification_metric_delta(
    fused: dict[str, Any], yolo: dict[str, Any]
) -> dict[str, Any]:
    delta: dict[str, Any] = {}
    ...
    if "top1_accuracy" in delta:
        delta["direction"] = (
            "improved"
            if delta["top1_accuracy"] > 0
            else ("regressed" if delta["top1_accuracy"] < 0 else "unchanged")
        )
    return delta
    ...
    for prediction in predictions_sorted:
        ...
        for gt_index, target in enumerate(gt_by_image.get(image_id, [])):
            ...
            if coerce_int(target.get("class_id")) != class_id:
                continue
            iou = (
                polygon_iou_approx_fn(polygon, target.get("polygon_xy", []))
                if polygon_iou_approx_fn
                else 0.0
            )
            if iou > best_iou:
                best_iou = iou
        ...
        if best_index is not None and best_iou >= iou_threshold:
            ...
            fp += 1
    fn = sum(
        max(0, len(targets) - len(matched.get(image_id, set())))
...
def verdict_field_count(verdict: dict[str, Any], field: str) -> int:
    cross_check = (
        verdict.get("yolo_cross_check", {}) if isinstance(verdict, dict) else {}
    )
...
def build_item_metric_preview(
    *,
    ...
    ground_truth_records_for_metric_fn = (
        ground_truth_records_for_metric_fn or ground_truth_records_for_metric
    )
    ground_truth_classification_records_for_metric_fn = (
        ground_truth_classification_records_for_metric_fn
        or ground_truth_classification_records_for_metric
    )
    ground_truth_segmentation_records_for_metric_fn = (
        ground_truth_segmentation_records_for_metric_fn
        or ground_truth_segmentation_records_for_metric
    )
    yolo_prediction_records_for_metric_fn = (
        yolo_prediction_records_for_metric_fn or yolo_prediction_records_for_metric
    )
    ...
    yolo_classification_predictions_for_metric_fn = (
        yolo_classification_predictions_for_metric_fn
        or yolo_classification_predictions_for_metric
    )
    fused_classification_predictions_for_metric_fn = (
        fused_classification_predictions_for_metric_fn
        or fused_classification_predictions_for_metric
    )
    yolo_segmentation_predictions_for_metric_fn = (
        yolo_segmentation_predictions_for_metric_fn
        or yolo_segmentation_predictions_for_metric
    )
    fused_segmentation_predictions_for_metric_fn = (
        fused_segmentation_predictions_for_metric_fn
        or fused_segmentation_predictions_for_metric
    )
    ...
    ground_truth_classes = ground_truth_classification_records_for_metric_fn(
        image_path, names
    )
    ground_truth_segments = ground_truth_segmentation_records_for_metric_fn(
        image_path, names
    )
    yolo_predictions = yolo_prediction_records_for_metric_fn(image_path, detections)
    fused_predictions = fused_prediction_records_for_metric_fn(
        image_path, fusion_preview
    )
    ...
    yolo_class_predictions = yolo_classification_predictions_for_metric_fn(
        image_path, detections
    )
    fused_class_predictions = fused_classification_predictions_for_metric_fn(
        image_path, verdict or {}, detections
    )
    yolo_class_metrics = evaluate_classification_metric_preview(
        yolo_class_predictions, ground_truth_classes
    )
    fused_class_metrics = evaluate_classification_metric_preview(
        fused_class_predictions, ground_truth_classes
    )
    yolo_seg_predictions = yolo_segmentation_predictions_for_metric_fn(
        image_path, detections
    )
    fused_seg_predictions = fused_segmentation_predictions_for_metric_fn(
        image_path, verdict or {}, detections
    )
    yolo_seg_metrics = evaluate_segmentation_metric_preview(
        yolo_seg_predictions,
        ground_truth_segments,
        polygon_iou_approx_fn=polygon_iou_approx_fn,
    )
    fused_seg_metrics = evaluate_segmentation_metric_preview(
        fused_seg_predictions,
        ground_truth_segments,
        polygon_iou_approx_fn=polygon_iou_approx_fn,
    )
    status = (
        "ok"
        if yolo_metrics.get("status") == "ok" or fused_metrics.get("status") == "ok"
        else "skipped"
    )
    return {
        "status": status,
        ...
        "classification": {
            ...
            "delta": classification_metric_delta(
                fused_class_metrics, yolo_class_metrics
            ),
        },
        "segmentation": {
...
def aggregate_metric_preview(
    ...
    *,
    ...
    for item in items:
        ...
        item_ground_truth_classes = ground_truth_classification_records_for_metric_fn(
            image_path, names
        )
        item_ground_truth_segments = ground_truth_segmentation_records_for_metric_fn(
            image_path, names
        )
        ...
        detector_summary = (
            item.get("detector", {}).get("summary", {})
            if isinstance(item.get("detector"), dict)
            else {}
        )
        if isinstance(detector_summary, dict):
            yolo_predictions.extend(
                yolo_prediction_records_for_metric_fn(image_path, [detector_summary])
            )
            yolo_class_predictions.extend(
                yolo_classification_predictions_for_metric_fn(
                    image_path, [detector_summary]
                )
            )
            yolo_seg_predictions.extend(
                yolo_segmentation_predictions_for_metric_fn(
                    image_path, [detector_summary]
                )
            )
        fusion_preview = (
            item.get("multimodal", {}).get("fusion", {})
            if isinstance(item.get("multimodal"), dict)
            else {}
        )
        ...
        llm_refine = (
            multimodal.get("llm_refine", {}) if isinstance(multimodal, dict) else {}
        )
        if isinstance(vlm, dict) and isinstance(vlm.get("verdict"), dict):
        ...
        if isinstance(detector_summary, dict):
            fused_class_predictions.extend(
                fused_classification_predictions_for_metric_fn(
                    image_path, verdict, [detector_summary]
                )
            )
...
def prediction_records_to_coco(
    records: list[dict[str, Any]], image_path: Path | None = None
) -> list[dict[str, Any]]:
...
def build_metric_guardrail(
    *,
    ...
    delta = (
        metric_preview.get("delta", {})
        if isinstance(metric_preview.get("delta"), dict)
        else {}
    )
    min_map_delta = float(
        multimodal_params.get("fusion_guardrail_min_map50_95_delta", 1e-6)
    )
    require_recall = parse_bool(
        multimodal_params.get("fusion_guardrail_require_recall_nonnegative"), True
    )
    ...
    material_change = len(fused_coco_records) != len(
        yolo_coco_records_for_items(
            items,
            yolo_prediction_records_for_metric_fn=yolo_prediction_records_for_metric_fn,
        )
    )
    if not material_change:
        fusion_summary = (
            metric_preview.get("fused", {})
            if isinstance(metric_preview.get("fused"), dict)
            else {}
        )
        yolo_summary = (
            metric_preview.get("yolo", {})
            if isinstance(metric_preview.get("yolo"), dict)
            else {}
        )
        material_change = any(
            (fusion_summary.get(key) != yolo_summary.get(key))
    ...
    if reasons:
        return {
            ...
            "reason": ",".join(reasons),
            "thresholds": {
                "min_map50_95_delta": min_map_delta,
                "require_recall_nonnegative": require_recall,
            },
            "delta": delta,
...
def aggregate_multimodal_evaluation(items: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for item in items:
        ...
        llm = multimodal.get("llm_refine", {}) or {}
        if (
            vlm.get("verdict_parse_status") == "parsed"
            or llm.get("verdict_parse_status") == "parsed"
        ):
            parsed += 1
        ...
        fusion_summary = (
            multimodal.get("fusion", {}).get("summary", {})
            if isinstance(multimodal.get("fusion"), dict)
            else {}
        )
    ...
    return {
        ...
        "avg_ground_truth_objects": round(total_gt_objects / gt_available, 4)
        if gt_available
        else None,
