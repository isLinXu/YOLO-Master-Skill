...
from .paths import YOLO_MASTER_ROOT
...
def _yaml_load(path: Path) -> Any:
    try:
        ...
        Exception
    ) as exc:  # pragma: no cover - dependency is part of the repository runtime
...
def _safe_roots(
    manifest_path: Path, artifact_root: str | Path | None
) -> tuple[Path, ...]:
    repository = YOLO_MASTER_ROOT
...
def _resolve_path(
    ...
    *,
    ...
    if not candidate.is_absolute():
        ...
        candidates.extend(
            [manifest_path.parent / candidate, *[root / candidate for root in roots]]
        )
...
def _compatibility_diagnostic(manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    checksum_valid = bool(expected) and str(expected) == manifest_checksum(
        dict(manifest)
    )
    if not has_schema:
...
def _record_file(
    ...
    *,
    ...
    path, error = _resolve_path(
        raw, manifest_path=manifest_path, roots=roots, relative_bases=relative_bases
    )
    if error:
    ...
    if not path.exists() or not path.is_file():
        ...
        return EvidenceRecord(
            kind, str(path), None, None, "missing", required, reason
        ), reason if required else None
    digest = _sha256(path)
    if declared_sha256 and str(declared_sha256) != digest:
        reason = f"checksum mismatch for {kind}: {path}"
        return EvidenceRecord(
            kind, str(path), digest, path.stat().st_size, "invalid", required, reason
        ), reason
    return EvidenceRecord(
        kind, str(path), digest, path.stat().st_size, "valid", required
    ), None


def _validate_payload(
    kind: str, record: EvidenceRecord, hard_failures: list[str]
) -> None:
    ...
    *,
    ...
    if not registry_path:
        return {
            "status": "unknown",
            "matched": False,
            "reason": "governance registry not provided",
        }
    registry, error = _resolve_path(
        registry_path, manifest_path=manifest_path, roots=roots
    )
    ...
    *,
    ...
    if compatibility["kind"] == "legacy_unversioned":
        hard_failures.append(
            "legacy source manifest is unversioned; migration required before publishing"
        )
    ...
    request = (
        ...
        if isinstance(manifest.get("request"), Mapping)
        else {}
    )
    inputs = (
        request.get("inputs", {}) if isinstance(request.get("inputs"), Mapping) else {}
    )
    result = (
        manifest.get("result", {})
    ...
    references = (
        ...
        if isinstance(environment.get("references"), Mapping)
        else {}
    )
    model_reference = (
        references.get("model", {})
        if isinstance(references.get("model"), Mapping)
        else {}
    )
    ...
    if compatibility["kind"] == "legacy_unversioned":
        job = (
            manifest.get("job", {}) if isinstance(manifest.get("job"), Mapping) else {}
        )
        save_dir_raw = job.get("save_dir")
        if save_dir_raw:
            save_dir, save_dir_error = _resolve_path(
                save_dir_raw, manifest_path=manifest_path, roots=roots
            )
            if save_dir_error:
    ...
    identity: dict[str, Any] = {
        "model": str(model_path) if model_path else _path_text(model_raw)
    }
    profile = (
        request.get("profile") if isinstance(request.get("profile"), Mapping) else None
    )
    if profile:
        ...
        if profile_path and profile.get("sha256"):
            profile_file, profile_error = _resolve_path(
                profile_path, manifest_path=manifest_path, roots=roots
            )
            if profile_error or profile_file is None or not profile_file.exists():
                hard_failures.append(
                    profile_error or f"missing profile YAML: {profile_path}"
                )
    ...
    if registry:
        ...
        if registry_error:
            (hard_failures if registry_record.status == "invalid" else missing).append(
                registry_error
                if registry_record.status == "invalid"
                else "governance_registry"
            )
    ...
    export_evidence: dict[str, Any] = {
        "format": result.get("export", {}).get("format")
        if isinstance(result.get("export"), Mapping)
        else None
    }
    if export_matrix:
        record, error = _record_file(
    ...
    adapter_requested = any(
        token in request_text for token in ("lora", "molora", "peft", "adapter")
    )
    for conditional_kind in sorted(_CONDITIONAL_KINDS):
        if conditional_kind in record_kinds:
            continue
        applicable = adapter_requested or (
            conditional_kind == "prune_manifest" and "prune" in request_text
        )
        records.append(
            EvidenceRecord(
                conditional_kind,
    ...
    if hard_failures:
        ...
        if not stable:
            reasons.append(
                f"governance status is not stable: {governance.get('status')}"
            )
    ...
    checkpoint = next(
        (
            item
            for item in records
            if item.kind == "checkpoint" and item.status == "valid"
        ),
        None,
    )
    if checkpoint:
    ...
    identity["model_yaml"] = next(
        (item.to_dict() for item in records if item.kind == "model_yaml"), None
    )
    return ReleaseBundle(
        source,
        identity,
...
def write_release_bundle(bundle: ReleaseBundle, path: str | Path) -> Path:
    ...
    descriptor, temporary_name = tempfile.mkstemp(
        prefix=".release_bundle.", suffix=".tmp", dir=target.parent
    )
    temporary = Path(temporary_name)
    try:
        with os.fdopen(descriptor, "w", encoding="utf-8") as handle:
            json.dump(
                bundle.to_dict(), handle, ensure_ascii=False, indent=2, sort_keys=True
            )
            handle.write("\n")
