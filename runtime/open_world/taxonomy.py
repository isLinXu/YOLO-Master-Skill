...
def taxonomy_token_set(label: str) -> set[str]:
    return {
        token
        for token in re.split(r"[^a-z0-9]+", normalize_open_world_label_text(label))
        if token
    }
...
def resolved_open_world_aliases(multimodal_params: dict[str, Any]) -> dict[str, str]:
    ...
    candidates = (
        [Path(alias_file)]
        if alias_file
        else [SKILL_ROOT / "assets" / "open_world_label_aliases.json"]
    )
            ...
            if candidate.exists():
                ...
                if isinstance(loaded, dict):
                    for key, value in loaded.items():
                        if key in (None, "") or value in (None, ""):
                            continue
                        aliases[normalize_open_world_label_text(str(key))] = str(
                            value
                        ).strip()
        except Exception:
            continue
...
def compact_open_world_profile(
    multimodal_params: dict[str, Any], user_prompt: str = ""
) -> str:
    explicit = (
        str(
            multimodal_params.get("compact_open_world_profile")
            or multimodal_params.get("open_world_profile")
            or ""
        )
        .strip()
        .lower()
...
def resolved_open_world_assist_profile(multimodal_params: dict[str, Any]) -> str:
    explicit = (
        str(multimodal_params.get("open_world_assist_profile") or "").strip().lower()
    )
...
def apply_open_world_assist_profile_defaults(
...
def effective_prompt_template_name(
    ...
    if not template_name:
        return None
    if provider_prefers_compact_open_world_schema(
        provider_cfg, prompt_template, multimodal_params
    ):
    ...
    *,
    ...
    effective_template = str(
        effective_prompt_template_name(
            provider_cfg, prompt_template, multimodal_params, user_prompt
        )
        or ""
    )
...
def taxonomy_match_score(
    label_info: dict[str, Any], candidate: dict[str, Any]
) -> float:
...
def _wordnet_hypernym_candidates(label: str) -> list[dict[str, Any]]:
    def static_candidates() -> list[dict[str, Any]]:
        ...
        for name in STATIC_HYPERNYM_FALLBACKS.get(
            normalize_open_world_label_text(label), []
        ):
...
def resolve_open_world_taxonomy_candidates(
    ...
    datasets = multimodal_params.get("open_world_taxonomy_datasets") or [
        "lvis",
        "v3det",
    ]
    dataset_list = [
        str(item).strip().lower() for item in as_list(datasets) if str(item).strip()
    ]
    topk = max(1, int(multimodal_params.get("open_world_taxonomy_topk", 5) or 5))
    min_score = float(
        multimodal_params.get("open_world_taxonomy_min_score", 40.0) or 40.0
    )
    require_exact_for_generic = parse_bool(
        multimodal_params.get("open_world_taxonomy_require_exact_for_generic"), True
    )
    enable_hypernym_fallback = parse_bool(
        multimodal_params.get("open_world_taxonomy_hypernym_fallback"), True
    )
    ...
    if not best and enable_hypernym_fallback:
        hypernym_candidates = _wordnet_hypernym_candidates(
            canonical or str(label_info.get("normalized_label") or "")
        )
...
def canonicalize_open_world_label(
    label: Any, multimodal_params: dict[str, Any]
) -> dict[str, Any]:
    ...
    if parse_bool(multimodal_params.get("open_world_label_normalizer"), True):
        result["taxonomy"] = resolve_open_world_taxonomy_candidates(
            result, multimodal_params
        )
    return result


def resolve_open_world_stats_policy(
    label_info: dict[str, Any], multimodal_params: dict[str, Any]
) -> dict[str, Any]:
    canonical = str(label_info.get("canonical_label") or "")
    category = str(label_info.get("category") or "")
    taxonomy = (
        label_info.get("taxonomy")
        if isinstance(label_info.get("taxonomy"), dict)
        else {}
    )
    filter_unmatched_taxonomy = parse_bool(
        multimodal_params.get("open_world_filter_unmatched_taxonomy"), True
    )
    filter_generic_labels = parse_bool(
        multimodal_params.get("open_world_filter_generic_labels"), True
    )
    include_in_stats = True
    reasons: list[str] = []
    if filter_generic_labels and (
        category == "generic" or canonical in OPEN_WORLD_GENERIC_LABELS
    ):
        include_in_stats = False
        reasons.append("generic_label")
    if (
        filter_unmatched_taxonomy
        and taxonomy
        and not parse_bool(taxonomy.get("matched"), False)
    ):
        include_in_stats = False
...
def normalize_open_world_prediction_items(
    ...
    for item in items:
        if not isinstance(item, dict):
            continue
        label_info = canonicalize_open_world_label(
            item.get("open_label") or item.get("label"), multimodal_params
        )
...
def normalize_possible_miss_items(
    items: Any, multimodal_params: dict[str, Any]
) -> list[dict[str, Any]]:
    normalized: list[dict[str, Any]] = []
...
def build_open_world_comparison_entry(
    *,
    ...
    raw_open_world = (
        fusion_preview.get("open_world_predictions_preview", [])
        if isinstance(fusion_preview, dict)
        else []
    )
    ...
    possible_misses = (
        normalize_possible_miss_items(
            cross_check.get("possible_misses"), multimodal_params
        )
    ...
    false_positives = (
        normalize_possible_miss_items(
            cross_check.get("false_positives"), multimodal_params
        )
        if isinstance(cross_check, dict)
        else []
    )
    fusion_summary = (
        fusion_preview.get("summary", {}) if isinstance(fusion_preview, dict) else {}
    )
    return {
...
def aggregate_open_world_comparison(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    reasoning_only_counts = {
        "open_world_predictions": 0,
        "possible_misses": 0,
        "false_positives": 0,
    }
    taxonomy_dataset_counts = {
        ...
        for item in entry.get("open_world_predictions", []) or []:
            if not parse_bool(item.get("include_in_open_world_stats"), True):
                ...
                for reason in as_list(item.get("open_world_filter_reasons")):
                    if reason:
                        filtered_reason_counts[str(reason)] = (
                            filtered_reason_counts.get(str(reason), 0) + 1
                        )
                continue
            label = (
                item.get("canonical_open_label")
                or item.get("open_label")
                or item.get("label")
            )
            if label:
            ...
            if isinstance(best, dict) and best.get("dataset"):
                ...
                taxonomy_dataset_counts["open_world_predictions"][dataset] = (
                    taxonomy_dataset_counts["open_world_predictions"].get(dataset, 0)
                    + 1
                )
                taxonomy_label = best.get("normalized_name") or best.get("name")
                if taxonomy_label:
                    taxonomy_label_counts[str(taxonomy_label)] = (
                        taxonomy_label_counts.get(str(taxonomy_label), 0) + 1
                    )
            else:
    ...
    for entry in entries:
        for candidate in entry.get("open_world_predictions", []) or []:
            ...
            for existing in merged:
                ...
                if (
                    not isinstance(existing_bbox, (list, tuple))
