...
def _env_present(primary: str) -> bool:
    return bool(
        ...
        or (primary != "OPENAI_API_KEY" and os.environ.get("OPENAI_API_KEY"))
    )
...
def openai_config(params: dict[str, Any]) -> dict[str, Any]:
    ...
    defaults = (
        provider_cfg.get("defaults")
        if isinstance(provider_cfg.get("defaults"), dict)
        else {}
    )
    api_key_env = str(
        params.get("openai_api_key_env")
...
def load_prompt_template(
    prompt_template: str | None, *, prompt_dir: Path
) -> str | None:
    ...
    template_path = prompt_dir / (
        candidate.name if candidate.suffix else f"{candidate.name}.md"
    )
...
def summarize_results_for_reasoning(
    results: Any, max_items: int = 5, max_boxes: int = 20
) -> list[dict[str, Any]]:
...
def multimodal_prompt_from_request(
...
def multimodal_overall_status(
    vlm_result: dict[str, Any], llm_result: dict[str, Any] | None
) -> str:
    vlm_status = vlm_result.get("status")
    if vlm_status == "blocked" or (
        llm_result and llm_result.get("status") == "blocked"
    ):
        return "blocked"
    if vlm_status != "ok":
        return "partial"
...
def build_visual_search_crop_prompt(
    base_prompt: str, region: dict[str, Any], detections: list[dict[str, Any]]
) -> str:
    return (
...
def default_visual_search_regions(
    detections: list[dict[str, Any]],
    *,
    normalize_detection_boxes_fn: Callable[
...
def run_visual_search_crop_passes(
    *,
    ...
    if mode in {"auto", "smart"}:
        visual_search = (
            initial_verdict.get("visual_search", {})
            if isinstance(initial_verdict, dict)
            else {}
        )
        if not (
            isinstance(visual_search, dict)
            and parse_bool(visual_search.get("needs_zoom"), False)
        ):
            return [], []
    ...
    for idx, region in enumerate(regions[:max_regions]):
        bbox = clamp_box_xyxy(
            region.get("bbox_xyxy", []), width, height, margin=crop_margin
        )
        if bbox is None:
            continue
