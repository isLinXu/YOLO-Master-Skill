...
def proposal_key(proposal: dict[str, Any]) -> str:
    ...
    bbox = valid_xyxy(
        proposal.get("bbox_xyxy") or proposal.get("bbox") or proposal.get("xyxy")
    )
    return f"bbox:{bbox}:{proposal.get('class_id')}:{proposal.get('label')}"


def resolve_proposal_reference(
    item: Any, proposals_by_id: dict[str, dict[str, Any]]
) -> dict[str, Any] | None:
...
def build_multimodal_fusion_preview(
    *,
    ...
    enabled = parse_bool(
        multimodal_params.get("fusion_enabled"),
        mode not in {"off", "none", "false", "0"},
    )
    if not enabled:
        return {"mode": "off", "strategy": "metric_safe_v1", "enabled": False}
    open_world_enabled = open_world_policy_enabled(
        multimodal_params
    ) or open_world_template_enabled(multimodal_params)
    open_world_assist = policy in {"open_world_assist", "open-world-assist"}
    allow_add = (
        policy in {"add_only", "balanced", "aggressive", "all"} or open_world_enabled
    )
    allow_suppress = policy in {"balanced", "aggressive", "all"} or (
        open_world_enabled
        and not open_world_assist
        and policy in {"open_world", "open-world"}
    )
    ...
    open_world_add_min = float(
        multimodal_params.get(
            "fusion_open_world_confidence_min", 0.55 if open_world_enabled else add_min
        )
    )
    add_require_unlinked = parse_bool(
        multimodal_params.get("fusion_add_require_unlinked"), True
    )
    add_max_linked_yolo_conf = float(
        multimodal_params.get("fusion_add_max_linked_yolo_confidence", 0.4)
    )
    add_allowed_bbox_quality = {
        str(v).lower()
        for v in as_list(
            multimodal_params.get("fusion_add_allowed_bbox_quality")
            or ["exact", "estimated"]
        )
        if v not in (None, "")
    }
    ...
    suppress_max_yolo_conf = float(
        multimodal_params.get("fusion_suppress_max_yolo_confidence", 0.45)
    )
    relabel_max_yolo_conf = float(
        multimodal_params.get("fusion_relabel_max_yolo_confidence", 0.5)
    )
    adjust_min_iou = float(multimodal_params.get("fusion_adjust_min_iou", 0.5))
    open_world_iou_relabel_enabled = parse_bool(
        multimodal_params.get("open_world_iou_relabel_enabled"), False
    )
    open_world_iou_relabel_threshold = float(
        multimodal_params.get("open_world_iou_relabel_threshold", 0.7) or 0.7
    )
    ...
    vlm_proposals = (
        [
            item
            for item in as_list(verdict.get("vlm_detections"))
            if isinstance(item, dict)
        ]
    ...
    def can_add(proposal: dict[str, Any]) -> tuple[bool, str]:
        ...
        if add_require_unlinked and linked_indices:
            ...
            if any(
                conf is not None and conf > add_max_linked_yolo_conf
                for conf in linked_confidences
            ):
                return False, "linked_yolo_confidence_too_high"
        return True, "vlm_add_candidate"

    for item in as_list(fusion_hints.get("suppress_yolo_indices")) + as_list(
        cross_check.get("false_positives")
    ):
    ...
    for item in as_list(fusion_hints.get("adjust_boxes")):
        ...
        for idx in idx_set:
            ...
            overlap = (
                box_iou_xyxy(original_box, bbox) if original_box is not None else None
            )
    ...
    for item in as_list(fusion_hints.get("relabel_yolo")):
        ...
        class_value = (
            ...
            else item.get("to_class_id")
        )
        class_value = (
            class_value if class_value is not None else item.get("new_class_id")
        )
    ...
    for proposal in vlm_proposals:
        action = str(
            proposal.get("coco_eval_action") or proposal.get("open_world_action") or ""
        ).lower()
        if action in {"add", "add_vlm", "add_vlm_detection", "new", "insert"}:
            if allow_add and (
                proposal_coco_class_id(proposal) is not None or not open_world_enabled
            ):
            ...
            action
            in {"open_world_add", "open-world-add", "open_world", "discover", "novel"}
            and allow_add
                    ...
                    actions.append(
                        {
                            ...
                            "reason": reason
                            if allow_suppress
                            else "fusion_policy_disallows_suppress",
    ...
    for item in yolo_boxes:
        ...
        if idx in relabels:
            ...
            prediction["action"] = (
                ...
                else f"{prediction['action']}+relabelled"
            )
    ...
    for proposal in explicit_adds:
        ...
        class_value = (
            proposal.get("class_id")
            if proposal.get("class_id") is not None
            else proposal.get("category_id")
        )
        class_id = coerce_int(class_value)
        if bbox is None or class_id is None:
            warnings.append(
                f"Skipped VLM proposal without usable bbox/class_id: {proposal.get('proposal_id', key)}"
            )
            continue
    ...
    for proposal in explicit_open_world_adds:
        ...
        if open_world_iou_relabel_enabled:
            for yolo_item in yolo_boxes:
                ...
                iou = (
                    box_iou_xyxy(yolo_bbox, bbox)
                    if isinstance(yolo_bbox, list)
                    else 0.0
                )
                if iou >= open_world_iou_relabel_threshold:
                    yolo_conf = coerce_float(yolo_item.get("confidence"))
                    if (
                        yolo_conf is not None
                        and yolo_conf <= open_world_iou_relabel_max_yolo_conf
                    ):
                        relabel_hit = {
            ...
            actions.append(
                {
                    ...
                    "reason": reason
                    if not allowed
                    else "open_world_confidence_below_threshold",
    ...
    summary = {
        ...
        "open_world_mapped_to_coco": sum(
            1 for item in fused_predictions if item.get("source") == "vlm_open_world"
        ),
