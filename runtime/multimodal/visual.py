...
def image_source_for_openai(
    source: Any, results_summary: list[dict[str, Any]], *, resolved_path
) -> str | None:
    candidates: list[str] = []
...
def encode_image_reference_for_openai(
    image_ref: str, *, resolved_path, max_bytes: int = 20_000_000
) -> dict[str, Any]:
    ...
    if not path.exists() or not path.is_file():
        raise FileNotFoundError(
            f"Image source for VLM is not a file or URL: {image_ref}"
        )
    size = path.stat().st_size
    if size > max_bytes:
        raise ValueError(
            f"Image source is too large for inline VLM upload: {path} ({size} bytes)"
        )
            ...
            if font is not None:
                ...
                draw.rectangle(
                    [
                        text_x,
                        text_y,
                        min(width, text_x + text_w + 6),
                        min(height, text_y + text_h + 4),
                    ],
                    fill=color,
                )
